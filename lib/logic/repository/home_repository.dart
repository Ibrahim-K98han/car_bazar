...
class HomeRepositoryImpl implements HomeRepository {
    ...
    try {
      ...
      final data =
          List<FeaturedCars>.from(
            cars.map((e) => FeaturedCars.fromMap(e)),
          ).toList();
      return Right(data);
    } on ServerException catch (e) {
      return Left(ServerFailure(e.message, e.statusCode));
  ...
  Future<Either<Failure, List<Dealers>>> getAllDealerList(Uri url) async {
    try {
      ...
      final data =
          List<Dealers>.from(dealer.map((e) => Dealers.fromMap(e))).toList();
      return Right(data);
    } on ServerException catch (e) {
      return Left(ServerFailure(e.message, e.statusCode));
    }
  ...
  Future<Either<Failure, DealerDetailsModel>> dealerDetails(
    ...
    try {
      final result = await remoteDataSource.getDealerDetails(
        langCode,
        userName,
      );
  ...
  @override
  Future<Either<Failure, CityModel>> getCity(Uri url, String id) async {
    try {
  ...
  @override
  Future<Either<Failure, CityModel>> getDealerCity(Uri url) async {
    try {
